...
class Naive_PairDataset(datasets.ImageFolder):
    def __init__(self, dir, num_NN, is_trn, batch_size, epoch, transform = None, *arg, **kw):
        super(Naive_PairDataset, self).__init__(dir, transform)
        self.num_NN = num_NN
        self.batch_size = batch_size
        self.num_nn_pos   = 30
        self.num_nn_neg_min = 1000
        self.num_nn_neg_red = 3000
        self.num_pos_max = 5
        self.epoch = epoch
        self.is_trn = is_trn
    ...
    def load_nn(dir):
        if os.path.isfile(os.path.join(dir, 'list_nn.pkl')):
            ...
            list_nn['trn'] = list_nn['trn']-1     # python base 0
            list_nn['val'] = list_nn['val']-1     # python base 0
            with open(os.path.join(dir,'dist_nn.pkl'),'rb') as f:
                dist_nn = pickle.load(f)
        
        else:
            # pairwise distance (training)
            fname_trn = os.path.join(dir,'dist_pose_trn.mat')
            fname_val = os.path.join(dir,'dist_pose_val.mat')
            pair_dist_trn = torch.from_numpy(sio.loadmat(fname_trn)['dist_pose_trn']).float()
            pair_dist_val = torch.from_numpy(sio.loadmat(fname_val)['dist_pose_val']).float()

            # sorting
            dist_nn_trn_sorted, list_nn_trn_sorted = torch.sort(pair_dist_trn, 1)
            dist_nn_val_sorted, list_nn_val_sorted = torch.sort(pair_dist_val, 1)
            
            # list of ordered nearest neighbors and distances
            list_nn_trn = list_nn_trn_sorted[:,1:]
            list_nn_val = list_nn_val_sorted[:,1:]
            dist_nn_trn = dist_nn_trn_sorted[:,1:]
            dist_nn_val = dist_nn_val_sorted[:,1:]
                
            # save
            list_nn = {}
            list_nn['trn'] = list_nn_trn
            list_nn['val'] = list_nn_val

            dist_nn = {}
            dist_nn['trn'] = dist_nn_trn
            dist_nn['val'] = dist_nn_val

            print('Now saving list_nn ... ', end='', flush= True)
            with open(os.path.join(dir, 'list_nn.pkl'), 'wb') as f:
                pickle.dump(list_nn, f)
            print('(Done)')
            print('Now saving dist_nn ... ', end='', flush= True)        
            with open(os.path.join(dir, 'dist_nn.pkl'), 'wb') as f:
                pickle.dump(dist_nn, f)
            print('(Done)')
    ...
    def generate_pairs(self, is_trn, imgs):
        ...
        if is_trn:
            ...
            # num_neg_all = max(num_data_trn - (self.num_nn_pos + 1) - (self.epoch  * self.num_nn_neg_red), self.num_nn_neg_min)
