...
parser.add_argument('--result-name', default='Logratio',
...
parser.add_argument('--loss', type=str, default='logratio',
                    help='Training loss name')
...
if args.loss.find('triplet')+1:
    if args.is_norm:
        LOG_DIR = args.log_dir + '/{}_pairwise_{}_{}_{}/fullbody_{}_lr{}_lrd{}_bat{}_emb{}'\
            .format(args.model,'norm', args.sampling, args.result_name, args.optimizer, args.lr, args.lr_decay, 
            args.batch_size, args.embedding_size)
...
def main():
    ...
    for epoch in range(start, end):
        ...
        if (args.sampling =='naive'):
            train_dir = Naive_PairDataset(dir=args.dataroot, is_trn = True,
...
def naive_train(train_loader, model, optimizer, epoch):
    ...
    for batch_idx, (input_pairs, img_idx) in pbar:
        ...
        if args.result_name.find('triplet')+1:
            loss = Naive_TripletLoss().forward(triplets)
