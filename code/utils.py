...
def eval_retrieval(dir, embed_all, num_data_val):
    ...
    for qidx in range(num_query):
        ...
        test1_embed.append(np.divide(np.cumsum(list_qdist),np.arange(1, num_max_NN+1)))
    test1_embed = np.mean(test1_embed, 0) * (224 / 288)
    ...
    test3_embed = np.ndarray([num_query, len(list_p)])
    for pidx in range(len(list_p)):
        ...
        for qidx in range(num_query):
            ...
            test3_embed[qidx][pidx] = dcg_embed / dcg_ideal
    test3_embed = np.mean(test3_embed, 0)

    return test1_embed, test2_embed, test3_embed
